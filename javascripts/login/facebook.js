window.fbAsyncInit = function() {
	FB.init({
        //appId: '884095405023668',
        appId: '137570657010962', //use rocket cloud
...
function runAPI() {
	FB.api('/me',{fields: fields}, function(response) {
		...
		var data = {
		 	...
		 	"clientId": clientId, 
            "publicId": response.email || response.id,
		 	"loginType": "social"
		 };
						    ...
						    success: function(data, textStatus, request){
						    	...
						    	if ( userAgent.toLowerCase().indexOf("iphone") != -1 || userAgent.toLowerCase().indexOf("ipad") != -1) {
									...
									if(window.history.back() != undefined){
										window.location = window.history+"?dataReturn="+responseJsonParameters;
										//window.history.back();
									}else{
